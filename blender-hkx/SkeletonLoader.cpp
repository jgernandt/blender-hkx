...
iohkx::SkeletonLoader::SkeletonLoader(const HavokEngine& engine)
{
...
void iohkx::SkeletonLoader::load(int argc, char** argv)
{
	for (int i = 0; i < argc; i++) {
		//Load the file
		hkSerializeUtil::ErrorDetails errorDetails;
		hkRefPtr<hkResource> res = hkSerializeUtil::load(argv[i], &errorDetails);
		if (!res)
			throw Exception(ERR_INVALID_SKELETON, errorDetails.defaultMessage.cString());

		res->removeReference();//starts at 1
		
		//Locate the skeleton
		hkRootLevelContainer* root = res->getContents<hkRootLevelContainer>();
		if (!root)
			throw Exception(ERR_INVALID_SKELETON, "No root level container");

		hkaAnimationContainer* animCtnr = root->findObject<hkaAnimationContainer>();
		if (!animCtnr)
			throw Exception(ERR_INVALID_SKELETON, "No animation container");

		if (animCtnr->m_skeletons.getSize() == 0)
			throw Exception(ERR_INVALID_SKELETON, "No skeleton");
